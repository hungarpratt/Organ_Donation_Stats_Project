...
ggplot(belief_means, aes(x = mean, y = Belief, color = Willing_f)) +
  ...
  theme_minimal(base_family = "plex", base_size = 14) +
  theme(
    panel.grid.minor = element_blank(),
    plot.margin = margin(t = 20, r = 20, b = 80, l = 20),
    plot.title = element_text(family = "plex", face = "bold", size = 16),
    axis.title = element_text(family = "plex"),
    axis.text  = element_text(family = "plex"),
    legend.text  = element_text(family = "plex"),
    legend.title = element_text(family = "plex", face = "bold"),
    # Optional: adjust spacing between axis and labels
    axis.title.x = element_text(margin = margin(t = 12, b = 10)),
